...
c = c.replace(/const handleLogoUpload = \(e: React\.ChangeEvent<HTMLInputElement>\) => \{[\s\S]*?\}\n\s*\}/, `const handleLogoUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
...
// Replace the Logo uploading UI block
const newLogoUi = `
              {/* Branding and Logos */}
...
// Replace the old Logo ui
c = c.replace(/\{\/\* Logo \*\/\}[\s\S]*?(?=\{\/\* Colors \*\/\})/, newLogoUi);

fs.writeFileSync('app/ajustes/page.tsx', c, 'utf8');
console.log("Updated ajustes branding section");
