...
const regex = /<div key=\{tx\.id\} className="bg-transparent py-4 border-b border-slate-300 dark:border-slate-800 flex flex-col md:flex-row gap-4 justify-between last:border-0">[\s\S]*?(?=<\/div>\s*\}\)\}\s*<\/div>)/;

const newBlock = `<div key={tx.id} className="bg-transparent py-3 border-b border-slate-200 dark:border-slate-800 flex flex-col md:flex-row gap-3 justify-between items-center last:border-0 hover:bg-slate-50/50 dark:hover:bg-slate-900/20 transition px-2 -mx-2 rounded-lg">
                ...
                <div className="flex flex-row md:flex-col justify-between items-center md:items-end w-full md:w-auto gap-2 md:gap-1 mt-2 md:mt-0">
                  <div className="flex flex-row md:flex-col items-center md:items-end gap-2 md:gap-0">
                    ...
                    <p className="text-[9px] text-slate-500 uppercase font-medium mt-0.5">{tx.paymentMethod}</p>
                  </div>
...
if (c.match(regex)) {
  ...
  console.log("Card layout replaced successfully.");
} else {
